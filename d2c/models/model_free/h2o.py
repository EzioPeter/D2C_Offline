...
class H2OAgent(BaseAgent):
    ...
    def _get_modules(self) -> utils.Flags:
        ...
        def log_alpha_prime_net_factory():
            log_alpha_prime = torch.ones(1, requires_grad=True, device=self._device)
            return log_alpha_prime
    ...
    def _build_optimizers(self) -> None:
        ...
        if self._cql_lagrange:
            self._alpha_prime_optimizer = utils.get_optimizer(opts.alpha_prime[0])(
                parameters=[self._log_alpha_prime_fn],
                lr=opts.alpha_prime[1],
                weight_decay=self._weight_decays,
            )            

    def _build_dsa_dsas_loss(self) -> Tuple[Tensor, Tensor, Dict]:
        ...
        real_state = _real_batch['s1']
        real_action = _real_batch['a1']
        real_next_state = _real_batch['s2']

        sim_state = _sim_batch['s1']
        sim_action = _sim_batch['a1']
        sim_next_state = _sim_batch['s2']
    ...
    def _build_alpha_loss(self, batch: Tuple[Dict, Dict]) -> Tuple:
        _real_batch, _sim_batch = batch
        real_states = _real_batch['s1']
        sim_states = _sim_batch['s1']

        df_states = torch.cat([real_states, sim_states], dim=0)

        with torch.no_grad():
            _, log_pi, _ = self._p_fn(df_states)
        alpha_loss = (-self._log_alpha_fn.exp() * (log_pi + self._target_entropy)).mean()
        self._alpha = self._log_alpha_fn.exp()
    ...
    def _build_q_loss(self, batch: Tuple[Dict, Dict]) -> Tuple[Tensor, Dict]:
        _real_batch, _sim_batch = batch
        real_states = _real_batch['s1']
        real_actions = _real_batch['a1']
        real_rewards = _real_batch['reward']
        real_next_states = _real_batch['s2']
        real_dones = _real_batch['done']

        sim_states = _sim_batch['s1']
        sim_actions = _sim_batch['a1']
        sim_rewards = _sim_batch['reward']
        sim_next_states = _sim_batch['s2']
        sim_dones = _sim_batch['done']        
        ...
        with torch.no_grad():
            real_next_state_actions, real_next_state_log_pi, _ = self._p_fn(real_next_states)
            real_qf1_next_target = self._q_target_fns[0](real_next_states, real_next_state_actions)
            real_qf2_next_target = self._q_target_fns[1](real_next_states, real_next_state_actions)
            real_min_qf_next_target = torch.min(real_qf1_next_target, real_qf2_next_target) - self._log_alpha_fn.exp() * real_next_state_log_pi
            if self._backup_entropy:
                real_target_q_value = (real_min_qf_next_target).view(-1) - self._log_alpha_fn.exp() * real_next_state_log_pi.view(-1)
            else:
                real_target_q_value = (real_min_qf_next_target).view(-1)
            real_next_q_value = real_rewards.flatten() + (1 - real_dones.flatten()) * self._discount * real_target_q_value

            sim_next_state_actions, sim_next_state_log_pi, _ = self._p_fn(sim_next_states)
            sim_qf1_next_target = self._q_target_fns[0](sim_next_states, sim_next_state_actions)
            sim_qf2_next_target = self._q_target_fns[1](sim_next_states, sim_next_state_actions)
            sim_min_qf_next_target = torch.min(sim_qf1_next_target, sim_qf2_next_target) - self._log_alpha_fn.exp() * sim_next_state_log_pi
            if self._backup_entropy:
                sim_target_q_value = (sim_min_qf_next_target).view(-1) - self._log_alpha_fn.exp() * sim_next_state_log_pi.view(-1)
            else:
                sim_target_q_value = (sim_min_qf_next_target).view(-1)
            sim_next_q_value = sim_rewards.flatten() + (1 - sim_dones.flatten()) * self._discount * sim_target_q_value
        ...
        real_qf1_a_values = self._q_fns[0](real_states, real_actions).view(-1)
        real_qf2_a_values = self._q_fns[1](real_states, real_actions).view(-1)
        sim_qf1_a_values = self._q_fns[0](sim_states, sim_actions).view(-1)
        sim_qf2_a_values = self._q_fns[1](sim_states, sim_actions).view(-1)

        real_qf1_loss = F.mse_loss(real_qf1_a_values, real_next_q_value)
        real_qf2_loss = F.mse_loss(real_qf2_a_values, real_next_q_value)

        if self._use_td_target_ratio:
            sqrt_IS_ratio = torch.clamp(self.real_sim_dynacmis_ratio(sim_states, sim_actions, sim_next_states), self._clip_dynamics_ratio_min, self._clip_dynamics_ratio_max).sqrt()
        else:
            sqrt_IS_ratio = torch.ones((sim_states.shape[0],)).to(self._device)

        sim_qf1_loss = F.mse_loss(sqrt_IS_ratio * sim_qf1_a_values, sqrt_IS_ratio * sim_next_q_value)
        sim_qf2_loss = F.mse_loss(sqrt_IS_ratio * sim_qf2_a_values, sqrt_IS_ratio * sim_next_q_value)
        ...
        if not self._use_value_regularization:
            ...
            if self._use_adaptive_weighting:
                u_sa = self.kl_sim_divergence(sim_states, sim_actions, sim_next_states)
            else:
                u_sa = torch.ones(sim_rewards.shape[0], device=self._device)
            omega = u_sa / u_sa.sum()
            std_omega = omega.std()

            if self._use_variant:
                sim_qf1_gap = (omega * sim_qf1_a_values).sum()
                sim_qf2_gap = (omega * sim_qf2_a_values).sum()
            else:
                sim_qf1_a_values += torch.log(omega)
                sim_qf2_a_values += torch.log(omega)
                sim_qf1_gap = torch.logsumexp(sim_qf1_a_values / self._cql_temp, dim=0) * self._cql_temp
                sim_qf2_gap = torch.logsumexp(sim_qf2_a_values / self._cql_temp, dim=0) * self._cql_temp

            qf1_diff = torch.clamp(
                sim_qf1_gap - real_qf1_a_values.mean(),
                self._cql_clip_diff_min,
                self._cql_clip_diff_max,
            )
            qf2_diff = torch.clamp(
                sim_qf2_gap - real_qf2_a_values.mean(),
                self._cql_clip_diff_min,
                self._cql_clip_diff_max,
            )
            ...
            if self._cql_lagrange:
                ...
                df_state = torch.cat([real_states, sim_states], dim=0)
        ...
        info['real_Q1'] = real_qf1_a_values.detach().mean()
        info['real_Q2'] = real_qf2_a_values.detach().mean()
        info['sim_Q1'] = sim_qf1_a_values.detach().mean()
        info['sim_Q2'] = sim_qf2_a_values.detach().mean()
        info['real_Q_target'] = real_target_q_value.mean()
        info['sim_Q_target'] = sim_target_q_value.mean()
    ...
    def _build_p_loss(self, batch: Tuple[Dict, Dict]) -> Tuple[Tensor, Dict]:
        _real_batch, _sim_batch = batch
        real_states = _real_batch['s1']
        sim_states = _sim_batch['s1']
        ...
        pi, log_pi, _ = self._p_fn(df_states)
        qf1_pi = self._q_fns[0](df_states, pi)
        qf2_pi = self._q_fns[1](df_states, pi)
    ...
    def _optimize_dsa_dsas(self) -> Dict:
        dsa_loss, dsas_loss, info = self._build_dsa_dsas_loss()
